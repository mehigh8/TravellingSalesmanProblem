...
int solve_tsp(graph_t *graph)
{
	...
	#pragma omp parallel for reduction(min:best_cost) private(node_idx)
	for (int second_node = 0; second_node < graph->node_count; second_node++) {
		if (second_node == graph->start_node)
			continue;
